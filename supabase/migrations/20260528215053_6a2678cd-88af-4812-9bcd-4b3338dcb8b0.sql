...
CREATE OR REPLACE FUNCTION public.broadcast_next_lead_batch(
  ...
  _ring_index integer DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Resolve sequential ring bands: 0→1, 1→2, 2→5, 5→10
  IF _ring_index IS NOT NULL THEN
    CASE _ring_index
      ...
      ELSE
        -- Beyond standard rings: treat as expansion ring up to user_radius (or 50 cap)
        v_ring_min := 10;
