...
BEGIN
  ...
  WITH candidate_vendors AS (
    SELECT
      ...
      CASE
        WHEN l.lat IS NOT NULL AND l.lng IS NOT NULL AND v.lat IS NOT NULL AND v.lng IS NOT NULL THEN
  ...
  SELECT count(*) INTO matched_count FROM inserted;

  UPDATE public.leads
  SET accepted_vendor_ids = (
      SELECT ARRAY(
        SELECT DISTINCT x
        FROM unnest(COALESCE(public.leads.accepted_vendor_ids, ARRAY[]::uuid[]) || COALESCE((SELECT array_agg(vendor_id) FROM inserted), ARRAY[]::uuid[])) AS x
      )
    ),
...
DROP TRIGGER IF EXISTS trg_prevent_customer_privileged_self_update ON public.customers;
CREATE TRIGGER trg_prevent_customer_privileged_self_update
BEFORE UPDATE ON public.customers
FOR EACH ROW
EXECUTE FUNCTION public.prevent_customer_privileged_self_update();

CREATE OR REPLACE FUNCTION public.prevent_merchant_link_privileged_self_update()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  IF auth.uid() IS NOT NULL AND auth.uid() = OLD.user_id AND NOT public.is_admin_user(auth.uid()) THEN
    ...
    NEW.payment_enabled := OLD.payment_enabled;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_prevent_merchant_link_privileged_self_update ON public.merchant_link_settings;
CREATE TRIGGER trg_prevent_merchant_link_privileged_self_update
BEFORE UPDATE ON public.merchant_link_settings
FOR EACH ROW
EXECUTE FUNCTION public.prevent_merchant_link_privileged_self_update();
...
BEGIN
  IF auth.uid() IS NOT NULL AND auth.uid() = OLD.user_id AND NOT public.is_admin_user(auth.uid()) THEN
    ...
    NEW.payment_completed_at := OLD.payment_completed_at;
...
DROP TRIGGER IF EXISTS trg_prevent_vendor_privileged_self_update ON public.vendors;
CREATE TRIGGER trg_prevent_vendor_privileged_self_update
BEFORE UPDATE ON public.vendors
FOR EACH ROW
EXECUTE FUNCTION public.prevent_vendor_privileged_self_update();