...
-- Add group_name to leads to capture which parent variation group the customer chose
ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS group_name text;

-- Update broadcast RPC to route only to vendors mapped to items in the same group
  ...
  IF v_lead.is_remote THEN
    WITH eligible AS (
      ...
      WHERE v.status = 'active'
        ...
        -- Group-tag routing: when customer picked a parent group (e.g. "Women Tailor"),
        -- only vendors who have at least one mapped active item in that same group
        -- under the same sub-category qualify.
        AND (
