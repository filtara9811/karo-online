...
AS $$
  SELECT 'https://karoonline.in/api/public/whatsapp/send-lead'
$$;
  ...
  IF v_lead.is_remote THEN
    WITH eligible AS (
      ...
      WHERE v.status = 'active'
        ...
        AND NOT (v.user_id = ANY(COALESCE(v_lead.accepted_vendor_ids, ARRAY[]::uuid[])))
        ...
        AND (
          v_lead.group_name IS NULL
          OR EXISTS (
            SELECT 1
            FROM public.vendor_item_mappings vim2
            JOIN public.catalog_items ci2 ON ci2.id = vim2.item_id
            WHERE (vim2.vendor_id = v.user_id OR vim2.vendor_id = v.id)
              AND COALESCE(vim2.is_active, true) = true
              AND ci2.group_tag = v_lead.group_name
              AND (v_lead.sub_category_id IS NULL OR ci2.category_id = v_lead.sub_category_id)
          )
        )
        AND NOT EXISTS (
    ...
    IF array_length(v_picked, 1) IS NULL THEN
      RETURN jsonb_build_object('ok', true, 'done', true, 'reason', 'no_remote_online_vendors', 'vendor_ids', '[]'::jsonb, 'count', 0, 'notified', 0);
    END IF;
...
GRANT EXECUTE ON FUNCTION public.broadcast_next_lead_batch(uuid, integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.broadcast_next_lead_batch(uuid, integer, integer) TO authenticated, service_role;

UPDATE public.leads l
SET is_marketplace = true,
    marketplace_reason = CASE
      WHEN COALESCE(l.accepted_count, 0) > 0 THEN 'under_capacity'
      WHEN NOT EXISTS (SELECT 1 FROM public.lead_notifications n WHERE n.lead_id = l.id) THEN 'zero_vendors'
      ELSE 'no_acceptance'
    END,
    marketplace_at = COALESCE(l.marketplace_at, now())
WHERE l.status = 'new'
  AND COALESCE(l.accepted_count, 0) < COALESCE(l.max_slots, 5)
  AND COALESCE(l.is_marketplace, false) = false
