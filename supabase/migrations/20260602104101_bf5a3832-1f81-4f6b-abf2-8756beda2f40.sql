-- Ensure auto-accepted lead notifications are also marked as started for dashboard visibility
...
-- Backfill recent accepted notifications from auto-accept vendors so they appear on vendor dashboard immediately
UPDATE public.lead_notifications ln
SET vendor_started_at = COALESCE(ln.vendor_started_at, ln.responded_at, ln.created_at)
FROM public.vendors v
WHERE v.user_id = ln.vendor_id
  AND ln.status = 'accepted'
  AND ln.vendor_started_at IS NULL
  AND COALESCE(v.auto_accept_leads, false) = true
  AND ln.created_at >= now() - interval '7 days';

-- Re-create broadcast function with explicit self-exclusion and robust accepted_vendor_ids handling
