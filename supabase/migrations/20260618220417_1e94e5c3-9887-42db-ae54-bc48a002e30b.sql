...
BEGIN
  ...
  -- Fallback: still resolve to a generic landing record so we never hard-fail.
  IF _user_id IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'landing', jsonb_build_object('announcement_active', false)
    );
  END IF;

  SELECT id, name, phone, avatar_url, shop_name, referral_code, address
  ...
  SELECT (kyc_status = 'verified'),
         ...
         NULLIF(shop_name,''),
         NULLIF(city,'')
    INTO _vendor_kyc, _vendor_shop_url, _vendor_shop_name, _vendor_city
  ...
  RETURN jsonb_build_object(
    ...
    'merchant', jsonb_build_object(
      ...
      'address', COALESCE(_customer.address, _vendor_city),
      'city', _vendor_city
    ),
