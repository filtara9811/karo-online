ALTER TABLE public.merchant_link_settings
  ADD COLUMN IF NOT EXISTS yt_source text,
  ADD COLUMN IF NOT EXISTS yt_enabled boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS yt_products jsonb NOT NULL DEFAULT '{}'::jsonb;
...
BEGIN
  ...
  INSERT INTO public.merchant_link_settings (
    ...
    yt_source, yt_enabled, yt_products
  ) VALUES (
    _uid,
    ...
    NULLIF(_payload->>'yt_source',''),
    COALESCE((_payload->>'yt_enabled')::boolean, false),
    COALESCE(_payload->'yt_products', '{}'::jsonb)
  )
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    yt_source = CASE WHEN _payload ? 'yt_source' THEN EXCLUDED.yt_source ELSE public.merchant_link_settings.yt_source END,
    yt_enabled = CASE WHEN _payload ? 'yt_enabled' THEN EXCLUDED.yt_enabled ELSE public.merchant_link_settings.yt_enabled END,
    yt_products = CASE WHEN _payload ? 'yt_products' THEN EXCLUDED.yt_products ELSE public.merchant_link_settings.yt_products END,
    updated_at = now();
