...
USING (key = ANY (ARRAY['social_links','no_vendor_state','lead_defaults','vendor_app','vendor_onboarding_video','home_banners','home_videos']));

CREATE POLICY "Authenticated can view safe app settings"
ON public.app_settings FOR SELECT TO authenticated
USING ((key = ANY (ARRAY['social_links','no_vendor_state','lead_defaults','vendor_app','vendor_onboarding_video','home_banners','home_videos'])) OR is_admin_user(auth.uid()));