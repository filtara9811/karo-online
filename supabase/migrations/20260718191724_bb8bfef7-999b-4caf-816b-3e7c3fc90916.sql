...
AS $$
BEGIN
  -- Trusted database functions (auto-match / notification auto-accept) may update
  -- system-managed lead fields. App users cannot set this flag through normal row edits.
  IF current_setting('app.lead_system_update', true) = 'on' THEN
    RETURN NEW;
  END IF;
  ...
  -- Customer owner can update notes/details but not status transitions directly.
...
BEGIN
  ...
  -- Auto-matched notifications are already accepted by the system even if the
  -- vendor's personal auto-accept toggle is off.
  IF NOT _auto AND COALESCE(NEW.auto_matched, false) IS NOT TRUE THEN
    RETURN NEW;
  END IF;
  ...
  IF NEW.vendor_id = ANY(COALESCE(_lead.accepted_vendor_ids, ARRAY[]::uuid[])) THEN
    NEW.status := 'accepted';
    NEW.responded_at := COALESCE(NEW.responded_at, now());
    NEW.vendor_started_at := COALESCE(NEW.vendor_started_at, now());
    RETURN NEW;
  END IF;
  ...
  UPDATE public.leads
    SET accepted_vendor_ids = array_append(COALESCE(accepted_vendor_ids, ARRAY[]::uuid[]), NEW.vendor_id),
...
BEGIN
  ...
  SET accepted_vendor_ids = (
    ...
    accepted_at = CASE WHEN matched_count > 0 THEN COALESCE(public.leads.accepted_at, now()) ELSE public.leads.accepted_at END,
    status = CASE WHEN matched_count > 0 AND public.leads.status = 'new' THEN 'accepted' ELSE public.leads.status END,
    updated_at = now()
  WHERE id = _lead_id;
...
REVOKE ALL ON FUNCTION public.guard_lead_self_update_fields() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.auto_accept_on_notify() FROM PUBLIC;
