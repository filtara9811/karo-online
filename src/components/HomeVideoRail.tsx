...
export function HomeVideoRail() {
    ...
    return (
      <div className="flex gap-3 px-4 pt-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="shrink-0 w-[76%] max-w-[300px] aspect-video rounded-2xl bg-white/70 animate-pulse" />
        ))}
      </div>
    );
  }
  ...
  return (
    <section className="pt-4">
      ...
      <div className="flex gap-3 overflow-x-auto px-4 pb-1 snap-x snap-mandatory [scrollbar-width:none] [&::-webkit-scrollbar]:hidden overscroll-x-contain">
        {videos.map((v) => (
          <button
            key={v.id}
            onClick={() => open(v)}
            className="shrink-0 w-[76%] max-w-[300px] text-left active:scale-[0.97] transition snap-start"
          >
            ...
            <p className="mt-2 text-[13.5px] font-bold text-slate-900 truncate">{v.title}</p>
            {v.subtitle && <p className="text-[11.5px] text-slate-500 truncate">{v.subtitle}</p>}
          </button>
        ))}
      </div>
